...
def write_grayscale(filename, pixels):
    ...
    Args:
        ...
        pixels: A rectangular image stored as a sequences of rows.
                Each row must be an iterable series of integers
                 in the range of 0 - 255.

    Raises:
        ValueError: If any of the integer values are out of range.
        OSErro: If the file coldn't be written.
    """
    height = len(pixels)
    width = len(pixels[0])
    ...
    with open(filename, 'wb') as bmp:
        ...
        bmp.write(b'\x00\x00\x00\x00') # Little-endian iteger. Zero placeholder for now.
